...
var htmlText1 = "<!DOCTYPE html>\n\
<html lang=\"en\">\n\
  <head>\n\
    <meta charset=\"utf-8\" />\n\
    <title>VN-Canvas TryIt</title>\n\
  </head>\n\
  <body>\n\
    ...
    <script>TOC = [\"demo/demo-config.js\"];</script>\n\
    <script src=\"templates/canvastext-0.4.1.mod.js\"></script>\n\
    <script src=\"templates/vncanvas-0.4.js\"></script>\n\
	<script>\n";
var htmlText2 = "tryit = [\n\
  label, \"tryit\",\n\
  scene, {src:\"black\"},\n\
  overlay, {src:\"demo/title.png\", effect:\"dissolve\"},\n\
];\n";
var htmlText3 = "</script>\n\
    <script>\n\
      window.onload = function() {\n\
        Stage.Init(\"stage\", 640, 360);\n\
        Stage.script.Init(tryit);\n\
      }\n\
    </script>\n\
  </body>\n\
</html>";
...
function statusline(msg, color) {
	...
	window.status = msg;
	document.title = "VN-Canvas Tryit Editor - " + msg;
}
...
function startit() {
	if (document.getElementById("syntaxHilite").checked) {
		view.document.open();
		view.document.write(htmlText1+document.getElementById("code1").value+htmlText3);
		view.document.close();
	}
	else {
		view.document.open();
		view.document.write(htmlText1+codeMirror.getValue()+htmlText3);
		view.document.close();
	}
}
...
function reset() {
	document.getElementById("code1").value = htmlText2;
	codeMirror.setValue(htmlText2);
	startit();
